...
namespace Lakshya_Yatra
{
    public partial class Registration : Form
    {
        ...
        public void InitializeForm(bool formLoad = false)
        {
            try
            {
                ...
                if (Customer_ID > 0 && Bus_Master_ID > 0)
                {
                    ...
                    panelPrintTicket.Visible = btnPrint.Visible = true;
                   ...
                   // Auto_Time = ds.Tables[0].Rows[0]["Auto_Time"] as byte[];
                    ...
                    dtpNavratriDate.Value = Convert.ToDateTime(ds.Tables[0].Rows[0]["yatra_date"]);
                    ...
                    Utilities.Instance.WriteLog("Setting ticket labels");
                    lblCustomerID.Text = Customer_ID.ToString();
                    lblName.Text = txtFirstName.Text + " " + txtLastName.Text;
                    lblAddress.Text = txtAddress.Text;
                    lblVehicleNo.Text = cbBus.SelectedItem.ToString();
                    lblSeatNo.Text = cbSeatNo.SelectedItem.ToString();
                    lblYatraDate.Text = dtpNavratriDate.Value.ToString("dd-MMM-yyyy");
                    Utilities.Instance.WriteLog("Setting Ticket Image");
                    if (System.IO.File.Exists(imagesFolder + Customer_ID.ToString() + ".jpg"))
                    {
                        using (var bmpTemp = new Bitmap(imagesFolder + Customer_ID.ToString() + ".jpg"))
                        {
        ...
        private void ClearImages()
        {
            ...
            if (imgTicket.Image != null) imgTicket.Image = null;
        ...
        private void TransferContents()
        {
            if (dgvCustomers.SelectedRows.Count > 0)
            {
                ...
                if (System.IO.File.Exists(imagesFolder + customerID.ToString() + ".jpg"))
                    {
                        using (var bmpTemp = new Bitmap(imagesFolder + customerID.ToString() + ".jpg"))
                        {
                            img = new Bitmap(bmpTemp);
                            imgSnapShot.Image = img;
                        }
